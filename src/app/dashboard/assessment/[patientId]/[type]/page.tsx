...
import FunctionalHistoryChart from "@/components/assessment/FunctionalHistoryChart";
import AssessmentComparisonChart from "@/components/assessment/AssessmentComparisonChart";
import ImageUpload from "@/components/assessment/ImageUpload";
import DataTableCell from "@/components/assessment/DataTableCell";
import DataTable from "@/components/assessment/DataTable";
import FunctionalQuestionnaireBlock from "@/components/assessment/FunctionalQuestionnaireBlock";
...
function AssessmentContent() {
  ...
  if (isFinished) {
    ...
    const isGeriatricTarget = ['man', 'ves13', 'lbpq', 'brief'].includes(type) || (returnTo === 'afGeriatria');
    const isMmiiTarget = ['lysholm', 'womac', 'ikdc', 'aofas'].includes(type) || (returnTo === 'afMmii');

    const handleReturn = () => {
        if (returnTo && result) {
            if (isGeriatricTarget || isMmiiTarget) {
                const resultsKeyMapping: Record<string, string> = {
                    man: 'man_score', ves13: 'ves13_score', lbpq: 'lbpq_score', brief: 'brief_score',
                    lysholm: 'lysholm_score', womac: 'womac_score', ikdc: 'ikdc_score', aofas: 'aofas_score'
                };
                const fieldKey = resultsKeyMapping[type];
                if (fieldKey) {
